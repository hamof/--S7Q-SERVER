...
client.on('message', message => {
if(message.content === `${prefix}VVV`) {
  message.guild.createChannel(`S7Q Voice Online : ${message.guild.members.filter(g => g.voiceChannel).size} ` , "voice").then(c => {
   c.overwritePermissions(message.guild.id, {CONNECT: false});
    message.channel.send(`**S7Q Voice Online : ${message.guild.members.filter(g => g.voiceChannel).size}**`);
    setInterval(() => {
    c.setName(`S7Q Voice Online : ${message.guild.members.filter(g => g.voiceChannel).size} `)
    },1000);
  })
